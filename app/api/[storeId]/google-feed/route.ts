...
export async function GET(
      ...
      <channel>
        ...
        ${products
          ...
          <item>
            ...
            <g:link>${process.env.FRONTEND_STORE_URL}/product/${
              product.id
            }</g:link>
            ...
            <g:google_product_category>Media > Books > Children's Books</g:google_product_category>
            ...
            <g:custom_label_0>Вік: ${product.ageGroups
              .map(
                (age) =>
                  ageGroups.find((ageGroup) => ageGroup.id === age.ageGroupId)
                    ?.name
              )
              .join(', ')} роки</g:custom_label_0>
               <g:custom_label_1>Категорія: ${product.categories
                 .map(
                   (category) =>
  ...
  const response = new NextResponse(xml, { status: 200, headers: corsHeaders });
